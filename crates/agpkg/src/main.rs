//! agpkg — AginxOS package installer CLI (M26; docs/SYSTEM.md §6.1).
//!
...
use agpkg::{cmd_available, cmd_list, cmd_opt_in, cmd_rollback, cmd_sync, install_file, usage, Fail, Paths};

fn main() {
    let args: Vec<String> = std::env::args().skip(1).collect();
    let p = Paths::from_env();
    // one key, one chain: same pubkey agupd verifies updates with.
    let pubkey = agsign::AGUPD_PUBKEY_B64;
    ...
    match cmd {
        ...
        "install" => {
            ...
            match install_file(&p, name, Path::new(src), sha) {
                Ok(agpkg::Kind::Binary) => println!("agpkg: installed {name} ({sha})"),
                Ok(agpkg::Kind::Bundle { unit, .. }) => {
                    println!("agpkg: installed {name} bundle ({sha}) — skill{}", if unit { " + unit" } else { "" })
                }
                Err(f) => fail(f, json),
            }
        }
...
fn print_out(out: agpkg::CmdOut, json: bool) {
    if json {
...
/// Fail path: envelope on --json, agpkg-prefixed stderr + exit 1/2
