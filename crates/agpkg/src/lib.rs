//! agpkg lib — package install engine (M26, Rust rewrite of the sh v0).
//!
...
//!   bytes (agsign lib — the SAME key/chain as agupd updates) before a
...
//!   already scans) followed by `agctl reload`. Extra files ride under
...
//!
//! Paths are env-overridable (AGPKG_BINDIR etc.) for host tests; on the
...
impl Paths {
    pub fn from_env() -> Paths {
        Paths {
            bindir: envp("AGPKG_BINDIR", "/var/bin"),
            appdir: envp("AGPKG_APPDIR", "/var/apps"),
            ...
            agctl: envp("AGPKG_AGCTL", "/usr/bin/agctl"),
            apps_d: envp("AGPKG_APPS_D", "/etc/apps.d"),
        }
    }
}
...
/// for an explicit path argument / AGPKG_MANIFEST env — the adb dev
...
/// production passes agsign::AGUPD_PUBKEY_B64.
pub fn load_manifest(path: &Path, dev: bool, pubkey_b64: &str) -> Result<Vec<Entry>, Fail> {
    ...
    if dev {
        eprintln!("agpkg: {path_display} unsigned (explicit path — dev override)", path_display = path.display());
    } else {
        let sig = {
            ...
            std::fs::read_to_string(PathBuf::from(s))
                .map_err(|e| {
                    ...
                    .with_hint(format!(
                        "sign it on the host: agsign sign .local/keys/agupd.key {}",
                        path.display()
                    ))
                })?
        };
        agsign::verify_with_key(pubkey_b64, &body, &sig).map_err(|e| {
            Fail::new(
                ErrorType::Auth,
                "manifest_bad_sig",
...
/// table verbatim (agsvc scans /var/lib/agpkg/units — M16's overlay
...
/// Best-effort `agctl reload`: the unit file is on disk either way and
/// agsvc rescans on restart — a failed reload is a warning, never a
/// failed install.
fn reload_units(p: &Paths) {
    match Command::new(&p.agctl).arg("reload").status() {
        Ok(s) if s.success() => {}
        _ => eprintln!(
            "agpkg: warning: {} reload failed — unit picked up on next agsvc restart",
            p.agctl.display()
        ),
    }
}
...
/// Download via agdl (the phone's only TLS fetcher) with the gh-proxy
...
pub fn fetch_install(p: &Paths, entry: &Entry) -> Result<Kind, Fail> {
    ...
    let mut ok = Command::new(&p.agdl).arg(&entry.url).arg(&dl).status().map(|s| s.success()).unwrap_or(false);
    if !ok && entry.url.starts_with("https://github.com/") {
        eprintln!("agpkg: retrying via gh-proxy.com");
        ok = Command::new(&p.agdl)
    ...
    if !ok || !dl.exists() {
        let _ = std::fs::remove_file(&dl);
        return Err(io_fail("download_failed", format!("{}: agdl could not fetch {}", entry.name, entry.url))
...
pub fn cmd_sync(p: &Paths, manifest: Option<&Path>, pubkey_b64: &str) -> Result<i32, Fail> {
    ...
    if !path.exists() {
        println!("agpkg: no manifest {} — nothing to sync", path.display());
        return Ok(0);
    }
    ...
    for e in &entries {
        ...
        match cur {
            Some(_) => println!("agpkg: {} stale — downloading", e.name),
            None => println!("agpkg: {} missing — downloading", e.name),
        }
        match fetch_install(p, e) {
            Ok(_) => println!("agpkg: installed {} ({})", e.name, e.sha256),
            Err(f) => {
                eprintln!("agpkg: {} sync FAILED (kept previous): {}", e.name, f.message);
                rc = 1;
            }
        }
    }
...
pub fn cmd_opt_in(p: &Paths, name: &str, pubkey_b64: &str) -> Result<(), Fail> {
    ...
    if cur.as_deref() == Some(e.sha256.as_str()) {
        ...
        seed_app(p, name)?;
        println!("agpkg: {name} already installed — seeded launcher entry");
        return Ok(());
    }
    fetch_install(p, e)?;
    seed_app(p, name)?;
    println!("agpkg: opted in {name}");
    Ok(())
}
...
pub fn cmd_rollback(p: &Paths, name: &str) -> Result<(), Fail> {
    ...
    println!("agpkg: rolled back {name}");
    Ok(())
}
...
pub fn usage() -> &'static str {
    "usage: agpkg install <name> <src> <sha256> | sync [manifest] | available [manifest] [--json] \
...
mod tests {
    ...
    fn sync_stamp_legacy_heal_and_failure_paths() {
        ...
        // no stamp -> healed + stamped, no download attempted (agdl is
    ...
    #[test]
    fn sync_downloads_via_stub_agdl() {
        let root = tmp("fetch");
        let p = paths(&root);
        // stub agdl: copies a fixture to $2
        let payload = root.join("payload.bin");
        fs::write(&payload, b"NEWBIN").unwrap();
        let stub = root.join("agdl");
