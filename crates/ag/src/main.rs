// ag — AginxOS single-entry command router (M24, ARCH §3/§4).
//
// The command universe is a flat set of `ag-*` executables; the filename
// IS the route and the filesystem IS the registry — dropping an executable
// into a cmd dir registers the command, no rebuild, no central list.
// Resolution is stat-only on the fast path; dispatch is execve, so exit
// codes and signals pass straight through (this process IS the target).
//
// Builtins: bare `ag` / `ag help` → menu; `ag commands [--all|--json|
// --check]` → listing / D1 envelope / lint gate. Intercepts (before any
// target code runs): `--help`/`-h` anywhere in argv prints route help and
// exits 0 — the target is never executed (the Omarchy `update aur --help`
// class of incident); a bare call to a command whose `ag:args=` declares a
// required `<positional>` is refused with usage, exit 2; unknown commands
// exit 127 with did-you-mean.
...
/// normal 127 path so `ag nope --help` still teaches.
...
fn dispatch(words: &[String]) -> i32 {
    ...
    if let Some((route, path, rest)) = resolve::resolve_fast(words) {
        ...
        // Table tier: ag:name= / ag:alias= routes.
        let t = resolve::build_table();
...
fn refuse_bare(route: &str, path: &Path, m: &Meta) {
    eprintln!("ag: 'ag {route}' requires arguments");
    match &m.args {
        Some(a) => eprintln!("usage: ag {route} {a}"),
        None => eprintln!("usage: ag {route} …"),
    }
    eprintln!("path: {}", path.display());
    eprintln!("try: ag {route} --help");
}

fn unknown(words: &[String]) -> i32 {
    let t = resolve::build_table();
    eprintln!("ag: unknown command '{}'", words.join(" "));
...
fn exec(path: &Path, rest: &[String]) -> ! {
    let cpath = match CString::new(path.as_os_str().as_bytes()) {
        Ok(c) => c,
        Err(_) => {
            eprintln!("ag: path contains NUL: {}", path.display());
            exit(126)
        }
    };
    ...
    for a in rest {
        match CString::new(a.as_str()) {
            Ok(c) => cargs.push(c),
            Err(_) => {
                eprintln!("ag: argument contains NUL");
                exit(2)
            }
        }
    }
...
fn print_menu(all: bool) {
    ...
    let t = resolve::build_table();
    println!("ag — AginxOS command router (file-is-registry)");
    println!();
    println!("usage: ag <command> [args…]");
    println!("       ag <command> --help      command help (flag works anywhere)");
    println!("       ag commands --all | --json | --check");
    println!();
    ...
    if by_group.is_empty() {
        println!("no commands found (AG_CMD_PATH={})", cmd_path_env());
        return;
    }
...
fn print_help(route: &str, path: &Path, m: Option<&Meta>) {
    ...
    println!("ag {route} — {}", m.summary.as_deref().unwrap_or("(no ag:summary=)"));
    println!();
    match &m.args {
        Some(a) => println!("usage: ag {route} {a}"),
        None => println!("usage: ag {route} [args…]"),
    }
...
fn builtin_commands(flags: &[String]) -> i32 {
    ...
    for f in flags {
        match f.as_str() {
            ...
            "--check" => check = true,
            "--help" | "-h" => {
                println!("usage: ag commands [--all|--json|--check]");
            ...
            other => {
                eprintln!("ag commands: unknown flag '{other}'");
                eprintln!("usage: ag commands [--all|--json|--check]");
                return 2;
            }
        }
    }
...
/// .agmd sidecar, ag:exec target missing. Warning (still exit 0): group
/// not registered in groups.desc.
fn lint() -> i32 {
    let mut errs = 0usize;
    let t = resolve::build_table();
    for c in &t.collisions {
        eprintln!("ag check: collision: {c}");
        errs += 1;
    }
    let groups = meta::load_groups();
    ...
    if errs == 0 {
        println!("ag check: {} commands OK", t.entries.len());
        0
    } else {
        eprintln!("ag check: {errs} error(s)");
        1
    }
}

/// ag:exec targets are either absolute paths or bare names resolved
