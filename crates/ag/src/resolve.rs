// Route resolution (M24): the registry IS the filesystem. Two tiers —
// resolve_fast stats `ag-<joined-prefix>` files directly (no metadata read,
// no dir listing); when that misses, build_table lists every executable
// ag-* across the cmd dirs and reads their headers so ag:name=/ag:alias=
// routes resolve too. Dir order = precedence: AG_CMD_PATH defaults to
// /var/bin:/usr/bin so provisioned (writable) commands shadow baked ones,
// same later-wins precedent agsvc uses for its unit files.
...
pub fn cmd_path_env() -> String {
    std::env::var("AG_CMD_PATH").unwrap_or_else(|_| DEFAULT_CMD_PATH.to_string())
}
...
/// `ag cam shot extra` → ["cam-shot-extra", "cam-shot", "cam"]. The k
...
/// Every executable `ag-*` file across the cmd dirs, in dir order. A route
/// seen in an earlier dir hides the same filename in later ones. Dotfiles,
/// the router itself and `.agmd` sidecars never qualify.
pub fn scan() -> Vec<(String, PathBuf)> {
    ...
    for d in cmd_dirs() {
        ...
        for e in rd.flatten() {
            ...
            if !name.starts_with("ag-") || name.ends_with(".agmd") {
                continue;
            }
            let p = e.path();
            if !is_exec_file(&p) {
...
impl Entry {
    /// Effective route: ag:name= override, else the filename.
...
/// fallback resolution tier (ag:name= / ag:alias= routes are not visible
/// to stat-only lookup).
