...
//!
//! The allow match strips the kernel's `" (deleted)"` suffix: agpkg
//! replaces binaries with tmp+rename, so a still-running carrier keeps
...
mod tests {
    ...
    fn missing_file_is_empty_deny_all() {
        let d = testkit::tmp("agsecret-policy-missing");
        let p = Policy::load(&d.join("nope")).unwrap();
        assert!(p.env.is_empty());
        assert!(!p.allows("brain.primary", Some("/var/bin/aginx-carrier")));
        assert!(!p.allows("admin", Some("/usr/bin/agsecret")));
    }

    #[test]
    fn malformed_file_is_an_error_not_a_guess() {
        let d = testkit::tmp("agsecret-policy-bad");
        let f = d.join("policy");
    ...
    fn allow_is_exact_scope_and_exe() {
        ...
        assert!(p.allows("brain.primary", Some("/var/bin/aginx-carrier")));
        assert!(!p.allows("brain.primary", Some("/usr/bin/agsecret")));
        assert!(!p.allows("brain.primary", None), "no identity → deny");
        assert!(!p.allows("other.scope", Some("/var/bin/aginx-carrier")));
        // kernel " (deleted)" suffix (agpkg tmp+rename of a running exe)
        // normalizes to the same binary
        assert!(p.allows("brain.primary", Some("/var/bin/aginx-carrier (deleted)")));
    }

    #[test]
    fn json_shape_round_trips() {
        let d = testkit::tmp("agsecret-policy-json");
        let f = d.join("policy");
        std::fs::write(
            &f,
            br#"{"env":{"CHARTER_SK":"api.charter"},
                "allow":{"api.charter":["/var/bin/aginx-carrier"],
                         "admin":["/usr/bin/agsecret"]}}"#,
        )
        .unwrap();
        let p = Policy::load(&f).unwrap();
        assert_eq!(p.scope_for_env("CHARTER_SK"), Some("api.charter"));
        assert!(p.allows("admin", Some("/usr/bin/agsecret")));
    }
}
