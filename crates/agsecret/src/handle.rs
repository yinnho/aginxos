...
pub fn handle(store: &mut Store, policy: &Policy, peer: &Peer, req: &Value) -> Envelope {
    ...
    match op {
        ...
        "issue" => agio::fail_hint(
            ...
            "not_implemented",
            "token minting arrives with aginxmemory roaming (M37)",
...
mod tests {
    ...
    fn setup() -> (Store, Policy) {
        let d = testkit::tmp("agsecret-handle");
        let mut store = Store::load(&d.join("store")).unwrap();
        store.set("brain.primary", "sk-live-1");
        store.set("api.charter", "charter-hmac-key");
        let mut p = Policy::default();
        p.env.insert("AGINXBRAIN_API_KEY".into(), "brain.primary".into());
        p.env.insert("CHARTER_SK".into(), "api.charter".into());
        p.allow.insert(
            "brain.primary".into(),
            vec!["/var/bin/aginx-carrier".into()],
        );
        p.allow.insert(
            "api.charter".into(),
            vec!["/var/bin/aginx-carrier".into()],
        );
        p.allow.insert("admin".into(), vec!["/usr/bin/agsecret".into()]);
        (store, p)
    }

    fn carrier() -> Peer {
        Peer { uid: 0, exe: Some("/var/bin/aginx-carrier".into()) }
    }
    fn human() -> Peer {
        Peer { uid: 0, exe: Some("/usr/bin/agsecret".into()) }
    }
    fn stranger() -> Peer {
        Peer { uid: 0, exe: Some("/var/bin/some-other-cli".into()) }
    }

    #[test]
    fn get_allows_only_scope_owner() {
        let (s, p) = setup();
        let r = handle(&mut s.clone_store(), &p, &carrier(), &json!({"op":"get","scope":"brain.primary"}));
        ...
        // allowed exe, wrong scope → still denied
        let (s, p) = setup();
        let r = handle(&mut s.clone_store(), &p, &human(), &json!({"op":"get","scope":"brain.primary"}));
        assert_eq!(r["error"]["code"], json!("denied"));
    }
    ...
    fn env_maps_name_then_gates_scope() {
        let (s, p) = setup();
        let r = handle(&mut s.clone_store(), &p, &carrier(), &json!({"op":"env","name":"AGINXBRAIN_API_KEY"}));
        assert_eq!(r["ok"], json!(true));
        assert_eq!(r["data"]["value"], json!("sk-live-1"));
        ...
        let (s, p) = setup();
        let r = handle(&mut s.clone_store(), &p, &carrier(), &json!({"op":"env","name":"NOPE"}));
    ...
    fn sign_returns_mac_not_key() {
        let (s, p) = setup();
        let r = handle(&mut s.clone_store(), &p, &carrier(), &json!({"op":"sign","scope":"api.charter","string":"hello"}));
        assert_eq!(r["ok"], json!(true));
        let mac = r["data"]["mac"].as_str().unwrap().to_string();
        assert_eq!(mac.len(), 64, "sha256 hex");
        assert_eq!(mac, hmac_hex(b"charter-hmac-key", "hello"));
        // the response never contains the key material
        assert!(!serde_json::to_string(&r).unwrap().contains("charter-hmac-key"));

        let (s, p) = setup();
        let r = handle(&mut s.clone_store(), &p, &stranger(), &json!({"op":"sign","scope":"api.charter","string":"hello"}));
        assert_eq!(r["error"]["code"], json!("denied"));
    }
    ...
    fn put_rm_list_are_admin_only() {
        let (mut s, p) = setup();
        let r = handle(&mut s, &p, &carrier(), &json!({"op":"put","scope":"x.y","value":"v"}));
        assert_eq!(r["error"]["code"], json!("denied"), "carrier is not admin");

        let d = testkit::tmp("agsecret-handle-put");
    ...
    impl CloneStore for Store {
        fn clone_store(&self) -> Store {
            let d = testkit::tmp("agsecret-handle-clone");
