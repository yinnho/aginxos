//! agsecret — the human/admin face of the secret sidecar (M36).
//!
//! Router face is the /usr/bin/ag-secret shim (`ag secret …`). `set`
//! reads the value from STDIN, never from argv — argv is world-readable
//! through ps and lives in shell history. Output is the D1 envelope on
//! stdout (this is also the face the m36 suite parses); stderr carries
//! nothing by design.
...
use agsecret::client::{default_socket, request};
    ...
    match request(&sock, &req) {
        ...
        Err(e) => {
            agio::exit_fail(
                ...
                "sidecar_unreachable",
                &format!("{e} — is agsecretd running? (agctl status agsecretd)"),
            )
        }
    }
}
...
fn usage_exit() -> ! {
    eprintln!("usage: agsecret set <scope>   (value on stdin)");
    eprintln!("       agsecret get <scope>");
    eprintln!("       agsecret env <name>");
    eprintln!("       agsecret rm <scope>");
    eprintln!("       agsecret list");
    eprintln!("       agsecret sign <scope> <string>");
    std::process::exit(2);
}
