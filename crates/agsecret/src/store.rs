...
//!
//! Scope names are dotted paths (`brain.primary`, `api.charter`,
//! `relay.main`) — the policy allowlist is written against them. The file
//! is 0600 root; writes go through tmp+rename so a crash never leaves a
//! half-written store. The store rides the state tar across rootfs swaps
//! but is excluded from ag-backup (DECISIONS §9).
...
mod tests {
    ...
    fn missing_file_is_empty_store() {
        let d = testkit::tmp("agsecret-store-missing");
    ...
    fn round_trip_and_mode() {
        use std::os::unix::fs::PermissionsExt;
        let d = testkit::tmp("agsecret-store-rt");
        let p = d.join("store");
        let mut s = Store::load(&p).unwrap();
        s.set("brain.primary", "sk-test-1");
        s.set("api.charter", "hmac-secret");
        s.save().unwrap();
        ...
        assert_eq!(back.scopes(), vec!["api.charter", "brain.primary"]);
        assert!(back.remove("api.charter"));
        assert!(!back.remove("api.charter"));
        back.save().unwrap();
    ...
    fn corrupt_store_is_invalid_data() {
        let d = testkit::tmp("agsecret-store-bad");
        let p = d.join("store");
