//! agsecret — the secret sidecar (M36; SYSTEM.md §7.1, DECISIONS §9).
//!
//! One daemon (`agsecretd`) holds every boundary credential; consumers
//! (carrier daemon, `ag api`, future relay/memory faces) ask over a unix
//! socket instead of inheriting `~/.aginx/carrier/.env` into every spawned
//! CLI. Wire shape: one ndjson request line in, one agio envelope line out.
//!
...
//!
//! Layout:
//! - [`store`] — the 0600 JSON store at /var/lib/ag/secret/store
...
/// EXCLUDED from ag-backup: the #86 push half must not bootstrap from its
/// own cargo. Lost keys are re-entered (`ag secret set`).
pub const DEFAULT_STORE: &str = "/var/lib/ag/secret/store";
...
pub const DEFAULT_LOG: &str = "/var/log/agsecretd.log";
...
mod tests {
    ...
    fn defaults_are_stable_paths() {
        ...
        assert_eq!(super::DEFAULT_STORE, "/var/lib/ag/secret/store");
