...
//! sidecar. Deliberately dependency-free beyond serde_json — the wire
//! protocol IS the interface, no cross-repo crate (M36 spec).
//!
//! Consumer resolution order (SYSTEM.md §7.1): real env var > sidecar >
//! `.env` file. This crate only provides the sidecar leg; the caller owns
//! the order. Failures are transport-level (socket gone, malformed reply)
//! and should read as "sidecar absent", never as "secret missing" — a
...
/// Default socket, or the `AGSECRET_SOCKET` override (adb dev loop and
/// tests — same pattern as AG_CMD_PATH/AGPKG_MANIFEST).
pub fn default_socket() -> PathBuf {
    std::env::var_os("AGSECRET_SOCKET")
        .map(PathBuf::from)
...
mod tests {
    ...
    fn socket_override_env() {
        ...
        // AGSECRET_SOCKET wins when set; the default is the boot socket.
        let d = testkit::tmp("agsecret-client-sock");
        let s = d.join("s.sock");
        std::env::set_var("AGSECRET_SOCKET", &s);
        assert_eq!(default_socket(), s);
        std::env::remove_var("AGSECRET_SOCKET");
