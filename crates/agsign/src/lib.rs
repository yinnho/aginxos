//! agsign lib — ed25519 验签面（M26 从 agupd 内联抽出，一把钥匙一条链）。
//!
...
//! keys/，gitignored），轮换走"旧钥匙签的更新里嵌新钥匙"（v2 问题）。
//!
//! 消费方：agupd（更新 manifest）、agpkg（包 manifest，M26 起）、
//! agsign CLI（keygen/sign/verify，主机侧）。
//!
//! **公钥挪到这里后，agupd/agpkg 都不再内嵌自己的副本**——升钥匙改一处。

use base64::engine::general_purpose::STANDARD as B64;
use base64::Engine;
use ed25519_dalek::{Signature, Verifier, VerifyingKey};

/// 更新签名公钥（ed25519，base64）。与 .local/keys/agupd.key 配对；
/// 轮换 = 发一个用它签的更新、更新里带新公钥（一期单钥匙）。
pub const AGUPD_PUBKEY_B64: &str = "N/qhN+0s1P0GOJjvpcyQjxAYmwHT00z09p2+6JA5nns=";
...
pub fn verify(body: &[u8], sig_b64: &str) -> Result<(), String> {
    verify_with_key(AGUPD_PUBKEY_B64, body, sig_b64)
}
...
mod tests {
    ...
    fn builtin_key_is_the_update_key() {
        ...
        // 验收里覆盖，这里只锁"内置钥匙可解析"。
        assert_eq!(B64.decode(AGUPD_PUBKEY_B64).unwrap().len(), 32);
    }
}
