//! agdone — provision 的 done 标记纪律（M27，docs/SYSTEM.md §9）。
//!
//! 一次性引导步骤（M28 的 python/pip finalize 是第一个真实租户）需要
//! "成功才留痕"的标记：步骤失败不写标记，下次开机自然重试；标记坏了
//! （非常规文件、不可读）当不存在。态落 /var/lib/ag/done/<name>，随
//! agupd 的 state tar 存活换机（M26 起 /var/lib 在包内）。
//!
//! 约定：
...
//!     agdone check python-finalize || { pip install -r … && agdone mark python-finalize; }
...
pub const DEFAULT_DIR: &str = "/var/lib/ag/done";

/// The marker directory (AGDONE_DIR overrides for tests/dev loops).
pub fn dir() -> PathBuf {
    std::env::var_os("AGDONE_DIR").map(PathBuf::from).unwrap_or_else(|| PathBuf::from(DEFAULT_DIR))
}
