//! agdone CLI — provision 的 done 标记（M27；lib.rs 有语义与测试）。
//!
//! 退出码三分：0=成功/已标记，3=check 的"未标记"（查询的合法答案，
//! 不是失败——provision 用 `agdone check x || step && agdone mark x`），
...
use agdone::{is_marked, list, mark, marked_at, reset, reset_all, validate_name};

const NOT_MARKED: i32 = 3;

fn usage() -> &'static str {
    "usage: agdone [--json] <command> [args]
...
markers live in /var/lib/ag/done/<name>; content is the epoch seconds of
the mark (advisory). AGDONE_DIR overrides the directory (tests, dev)."
}
    ...
    match rest.as_slice() {
        ...
        ["check", name] => {
            ...
            if json {
                let env = agio::ok_meta(
                    ...
                    json!({"at": marked_at(name), "dir": agdone::dir().display().to_string()}),
                );
                agio::print(&env);
            } else if marked {
                println!("agdone: {name} marked at {}", marked_at(name).map(|t| t.to_string()).unwrap_or_else(|| "?".into()));
            } else {
                println!("agdone: {name} not marked");
            }
            ...
            match mark(name) {
                Ok(at) => {
                    if json {
                        ...
                        println!("agdone: marked {name} at {at}");
                    }
                    exit(0);
                }
        ...
        ["ensure", name] => {
            ...
            if is_marked(name) {
                if json {
                    ...
                    println!("agdone: {name} already marked at {}", marked_at(name).map(|t| t.to_string()).unwrap_or_else(|| "?".into()));
                }
                exit(0);
            }
            match mark(name) {
        ...
        ["reset", "--all"] => {
            ...
            if json {
                ...
                println!("agdone: removed {n} marker{}", if n == 1 { "" } else { "s" });
            }
            exit(0);
        }
        ["reset", name] => {
            ...
            if json {
                ...
                println!("agdone: {}", if removed { format!("removed {name}") } else { format!("{name} had no marker") });
            }
            exit(0);
        }
        ["list"] => {
            ...
            if json {
                ...
                for (n, at) in &all {
                    println!("agdone: {n}\t{}", at.map(|t| t.to_string()).unwrap_or_else(|| "?".into()));
                }
            }
            exit(0);
        }
