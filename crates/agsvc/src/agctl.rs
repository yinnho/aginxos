// agctl — control client for the agsvc supervisor (M16, docs/SYSTEM.md
...
use agsvc::{log_path, CTL_SOCK};

const USAGE: &str = "usage: agctl list | status <name> | start|stop|restart <name> | reload | logs <name>";
...
fn main() {
    ...
    let mut s = match UnixStream::connect(CTL_SOCK) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("agctl: connect {CTL_SOCK}: {e} (is agsvc running?)");
            std::process::exit(1);
        }
    };
    if let Err(e) = s.write_all(format!("{req}\n").as_bytes()) {
        eprintln!("agctl: write: {e}");
        std::process::exit(1);
    }
    let _ = s.shutdown(std::net::Shutdown::Write);
    let mut resp = String::new();
    if let Err(e) = s.read_to_string(&mut resp) {
        eprintln!("agctl: read: {e}");
        std::process::exit(1);
    }
