// agsvc — the AginxOS service supervisor (M16, docs/SYSTEM.md §12.1).
//
// busybox init stays PID 1 and respawns this process
// (`::respawn:/usr/bin/agsvc`); agsvc owns every other daemon. Children
// carry PR_SET_PDEATHSIG(SIGKILL), so an agsvc crash takes them down too —
// init respawns agsvc, which respawns everything else, with no
// double-spawn window.
//
// Readiness contract (Redox-init shape): `type = notify` spawns pass a
// pipe write end as fd 3 + env AGSVC_NOTIFY=3; one byte written = ready,
...
// `agctl start`. Missing binaries are `absent`, not failed — provisioning
...
impl Svc {
    ...
    fn try_spawn(&mut self, name: &str, now: Instant) {
        ...
        match spawn_unit(&unit) {
            ...
            Err(e) => {
                kmsg(&format!("agsvc: {} spawn failed: {e}\n", unit.name));
    ...
    fn failure(&mut self, name: &str, now: Instant, why: &str) {
        ...
        if park {
            kmsg(&format!(
                "agsvc: {display} breaker open ({why}, {} exits in {BREAKER_WINDOW_S}s)\n",
                r.fails.len()
            ));
            r.set_st(St::Failed);
        } else if restartable {
...
fn main() {
    ...
    let listener = match UnixListener::bind(CTL_SOCK) {
        Ok(l) => l,
        Err(e) => {
            kmsg(&format!("agsvc: bind {CTL_SOCK}: {e}\n"));
            std::process::exit(1);
        }
    };
    let _ = listener.set_nonblocking(true);
    let _ = std::fs::set_permissions(CTL_SOCK, Permissions::from_mode(0o600));
    if let Ok(mut f) = File::create("/run/agsvc.pid") {
