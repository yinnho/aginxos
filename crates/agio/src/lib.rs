// agio — D1 输出契约的信封发射器（ARCH §D1，M25）。
//
// 一切 ag-* CLI 的 stdout 都是同一个 JSON 信封，agent 端只需要一种解析器：
//
//   {"ok":true,  "data":…, "meta":{…}?}
//   {"ok":false, "error":{"type":"usage", "code":"<cmd 稳定码>",
//                         "message":"…", "hint":"…"?}}
//
// 约定（与 M24 `ag commands --json` 的既有形状同构，此处转正为库）：
...
//
// 本 crate 只依赖 serde_json，无 tokio/clap——给 musl static 小 CLI 共用
// （ag / ag-done / agpkg / 后续 agmem、agb）。carrier 是独立仓库，按形状
// 镜射（契约是 JSON 形状，不是代码依赖）；carrier 侧对齐在 M25 retrofit。
...
mod tests {
    ...
    fn fail_shape_and_codes() {
        let v = fail_hint(ErrorType::Usage, "missing_arg", "need <name>", "try: ag pkg list");
        assert_eq!(v["ok"], json!(false));
        assert_eq!(v["error"]["type"], json!("usage"));
        assert_eq!(v["error"]["code"], json!("missing_arg"));
        assert_eq!(v["error"]["hint"], json!("try: ag pkg list"));
